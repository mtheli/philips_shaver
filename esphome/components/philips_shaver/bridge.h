...
class ShaverCoordinator;  // forward — defined in coordinator.h

// HA-side glue: registers services, fires events, manages the connected
// binary sensor. Owns no GATT state — forwards everything to a
// ShaverCoordinator pointer. The Coordinator pushes events back through
// fire_event() / publish_connected().
class ShaverBridge : public Component, public api::CustomAPIDevice {
 public:
  void setup() override;
  void loop() override;
  ...
  // stream and the logger: filter can target a single bridge by suffix.
  ...
  const std::string &get_bridge_id() const { return this->bridge_id_; }
