...
> `"standalone"`; see [ESP Bridge Setup](SETUP.md) for the
...
| `payload` | string (hex) | Raw bytes, lowercase hex. **May be empty**: a successful read of a 0-byte value yields `payload=""` with **no** `error` field — callers must treat that as "read OK, no data", not as a missing reply. |
...
Since 1.11.0 the bridge may additionally request a shorter connection
interval on its own when several calls queue up while the link sits in a
power-save profile (connection-parameter boost, logged as
`Requesting conn-param boost …`). This is transparent to callers — no HA
action involved; see the [README](README.md#connection-parameter-boost-bridge--1110)
for the mechanics.
