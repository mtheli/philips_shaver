...
Use the template [`esphome/atom-lite.yaml`](atom-lite.yaml) (Atom
Lite, two devices) or [`esphome/esp32-generic.yaml`](esp32-generic.yaml)
...
[lazy-encrypt fix from v1.7.0](CHANGELOG.md) — it works around a
...
> i9000 / S-series, OneBlade 360) — see the [Unpairing Guide](../docs/UNPAIRING.md)
...
[`esphome/atom-lite.yaml`](atom-lite.yaml).

> [!NOTE]
> Each device should only be connected via **one** path — either Direct BLE
> or ESP Bridge, not both simultaneously. See the
> [Unpairing Guide](../docs/UNPAIRING.md) if the device is currently paired with your
> phone or HA host.
...
- The shaver only advertises while powered on or in pair-mode. Switch it on
  ...
  [Unpairing Guide](../docs/UNPAIRING.md) for the model-specific procedure), then
  click **Start pairing**
...
payload formats, capability flags), see [BLE_PROTOCOL.md](../docs/BLE_PROTOCOL.md).
...
into pair-mode (see the [Unpairing Guide](../docs/UNPAIRING.md) for the model-specific
