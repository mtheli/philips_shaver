...
device. Passing that id as ``via_device_id`` is what HA deprecates (#15): it
...
does hand out the unresolvable composite id from the report, and
...
from custom_components.philips_shaver import (
...
ESP_DEVICE_NAME = "shaver_bridge"  # as we store it
ESP_MDNS_NAME = "shaver-bridge"  # as ESPHome stores it
MAC_CONNECTION = (dr.CONNECTION_NETWORK_MAC, ESP_MAC)

# Verbatim from the log in #15: the id of a pre-migration composite device.
# No device carries it — that is the whole complaint.
...
def add_shaver_entry(hass) -> MockConfigEntry:
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
...
def test_pre_split_core_uses_the_plain_lookup(hass) -> None:
    ...
    found = _async_get_esphome_device(PreSplitRegistry(hass, esp_device), ESP_MAC, esp_entry)
...
def test_the_double_reproduces_the_deprecated_lookup(hass) -> None:
    """Without the scope, the lookup hands back exactly what #15 complains about.
...
def test_link_lands_on_the_esphome_node(hass, monkeypatch) -> None:
    ...
    esp_entry = add_esphome_entry(hass)
    bt_entry = add_bluetooth_entry(hass)
    shaver_entry = add_shaver_entry(hass)
    esphome_split, bluetooth_split = register_splits(hass, esp_entry, bt_entry)

    real = dr.async_get(hass)
    shaver_device = real.async_get_or_create(
        config_entry_id=shaver_entry.entry_id, identifiers={(DOMAIN, ADDRESS)}
    )
    bridge_device = real.async_get_or_create(
        config_entry_id=shaver_entry.entry_id,
    ...
    _async_link_via_esp_device(hass, shaver_entry)

    assert real.async_get(shaver_device.id).via_device_id == esphome_split.id
...
def test_missing_esphome_device_leaves_the_link_alone(hass) -> None:
    ...
    esp_entry = add_esphome_entry(hass)
    shaver_entry = add_shaver_entry(hass)
    shaver_device = dr.async_get(hass).async_get_or_create(
        config_entry_id=shaver_entry.entry_id, identifiers={(DOMAIN, ADDRESS)}
    )

    _async_link_via_esp_device(hass, shaver_entry)

    assert dr.async_get(hass).async_get(shaver_device.id).via_device_id is None
