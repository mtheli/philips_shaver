...
reports via info events (firmware >= 1.12.0). Older firmware reports
neither field — the sensor must stay unknown instead of guessing.
Ported from the Sonicare integration (v0.22.0).
"""

from types import SimpleNamespace

from custom_components.philips_shaver.sensor import PhilipsBridgeBuildSensor


def _sensor_with_transport(**fields) -> PhilipsBridgeBuildSensor:
    sensor = PhilipsBridgeBuildSensor.__new__(PhilipsBridgeBuildSensor)
...
def test_old_firmware_reports_nothing() -> None:
    # Pre-1.12.0 bridges never send the fields; the transport attributes
