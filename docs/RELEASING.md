...
📟 **No ESP bridge firmware change** — `MIN_BRIDGE_VERSION` stays 1.8.0. ([bridge changelog](https://github.com/mtheli/philips_shaver/blob/main/esphome/CHANGELOG.md))
```

**Title:** `vX.Y.Z — what it is about`, e.g.
*v0.26.0 — Pairing over a bridge with Home Assistant's Auto scanning*.
...
| Case | Line |
| :--- | :--- |
| No firmware change | `📟 **No ESP bridge firmware change** — \`MIN_BRIDGE_VERSION\` stays 1.8.0.` |
| New firmware, optional | `📟 **ESP bridge firmware vX.Y.Z (optional)** — \`MIN_BRIDGE_VERSION\` stays 1.8.0, so bridges on older firmware keep working; <what they miss>. ([bridge changelog](…))` |
...
`## v1.13.0 — 2026-07-26` therefore becomes `#v1130--2026-07-26` (the double
...
scanning links the core pull request that changed the default scanning mode.
...
| File | Role |
| :--- | :--- |
| `esphome/components/philips_shaver/VERSION` | The single source of truth. Compiled into the binary as a define, reported back through `ble_get_info`, and read straight from GitHub by the integration's update entity to decide whether a bridge is out of date. |
...
"Build-time change only — no firmware behavior change, no version bump".
...
2. `esphome/components/philips_shaver/VERSION` — bump on **any** firmware
